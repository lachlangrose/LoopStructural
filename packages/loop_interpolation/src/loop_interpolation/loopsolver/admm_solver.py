...
def admm_solve(
    ...
    def _solve_linsys(system_matrix, rhs, kwargs):
        ...
        if callable(solver_name):
            try:
                return np.asarray(solver_name(system_matrix, rhs, x0=x0_guess, **kwargs), dtype=float)
            except TypeError:
    ...
    for _i in progressbar(range(nmajor)):
        ...
        if active_set and Q.shape[0] > 0:
            ...
            if refresh:
                active_rows_prev = _active_rows(qx_val[:, 0], xmin, xmax, prev_active=active_rows_prev)
                ...
                history.append(
                    {
                        ...
                        "eps_dual": eps_dual,
                        "rho": float(rho),
                        "active_count": int(np.sum(active_rows_prev)) if active_set and active_rows_prev is not None else int(n_ie),
                    }
                )
