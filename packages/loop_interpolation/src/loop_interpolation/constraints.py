...
class ValueConstraint(BaseConstraint):
    ...
    def check_shapes(self):
        ...
        if self.points.shape[0] != self.values.shape[0]:
            raise ShapeError("Value constraint points and values must contain the same number of rows")
...
class GradientConstraint(BaseConstraint):
    ...
    def check_shapes(self):
        ...
        vector_label = "Normal vector (nx, ny, nz)" if self.is_normal else "Gradient vector (gx, gy, gz)"
...
class InequalityConstraint(BaseConstraint):
    ...
    def check_shapes(self):
        ...
        if self.points.shape[0] != self.bounds.shape[0]:
            raise ShapeError("Inequality constraint points and bounds must contain the same number of rows")
...
class InequalityPair(BaseConstraint):
    ...
    @model_validator(mode="after")
    def check_shapes(self):
        self._set_field("points", _ensure_float_array(self.points, "Inequality pairs constraint points"))
        self._set_field("pair_ids", _ensure_float_array(self.pair_ids, "Inequality pairs constraint ids"))
        ...
        if self.points.shape[0] != self.pair_ids.shape[0]:
            raise ShapeError("Inequality pairs constraint points and pair_ids must contain the same number of rows")
...
class InterfaceConstraint(BaseConstraint):
    ...
    def check_shapes(self):
        ...
        self._set_field("interface_ids", _ensure_float_array(self.interface_ids, "Interface constraint ids"))
        ...
        if self.points.shape[0] != self.interface_ids.shape[0]:
            raise ShapeError("Interface constraint points and interface_ids must contain the same number of rows")
