...
class FDFoldInterpolator(FiniteDifferenceInterpolator):
    ...
    def __init__(self, grid, fold: Optional[FoldEvent] = None, data={}):
    ...
    def setup_interpolator(self, **kwargs):
        ...
        if self.fold is None:
            raise RuntimeError(
                "FDFoldInterpolator: no fold event set. "
                "Assign self.fold before calling setup_interpolator."
            )
    ...
    def add_fold_constraints(
        ...
        * Anisotropic regularisation using directional second derivatives,
        ...
        fold_norm : float or None
            Target gradient magnitude in the fold-normal direction.
