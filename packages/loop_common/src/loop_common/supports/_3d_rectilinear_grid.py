...
class RectilinearGrid(StructuredGrid):
    ...
    *ynodes*, *znodes* (strictly increasing).
    ...
    def inside(self, pos: np.ndarray) -> np.ndarray:
        return np.all(
            (pos > self.origin[None, :]) & (pos < self.maximum[None, :]), axis=1
        )

    def position_to_cell_index(
        self, pos: np.ndarray
    ...
    def build_scaled_operator_rows(
        ...
        if pure:
            ...
            col_global_idx = self.global_node_indices(
                col_ijk_4.reshape(-1, 3)
            ).reshape(n_interior, 4)
